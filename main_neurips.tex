...
\begin{abstract}
Deep-learning models in clinical settings face two intertwined obstacles:
\textbf{Domain Generalization (DG)} across heterogeneous acquisition protocols,
and \textbf{Long-Tailed Visual Recognition (LT-VR)} on highly skewed disease
populations. We formally show that the gradient-alignment objective favored by
DG conflicts with the class-aware optimization required for LT-VR, producing a
smoothness--sharpness trade-off that degrades both. To resolve this, we
introduce the \textbf{Agentic Causal Disentanglement (ACD)} framework, a
modular architecture that integrates explicit clinical knowledge sourced
autonomously from sonographers, radiologists, and specialists as a causal
intervention. ACD orchestrates three agents (Clinical Reasoning, Causal
Disentanglement, Code Generation) and routes predictions through a
class-conditional cascade rather than concatenating features. Across
\textbf{10 medical-imaging datasets spanning 4 modalities}, ACD yields an
average \textbf{10.3\% improvement} over DG and LT baselines on
multi-domain and in-domain long-tailed tasks.
\end{abstract}
...
demographics~\cite{Lei22, Chen23, Li23a}. Such variations distort $P(X|Y)$ and
...
Although DG and LT-VR have been extensively studied in isolation, they coexist in real-world medical settings. Existing DG methods typically report improvements in macro-level accuracy and cross-domain performance; however, they often fail to preserve performance on rare classes under domain shift, as demonstrated in Table\ref{tab:perclass_f1}. For example, in Diabetic Retinopathy grading, which involves five classes where the fourth class (Proliferative DR) is rare, training a ViT-based SOTA ERM model on the APTOS dataset and evaluating on a different domain such as MESSIDOR leads to a drop in overall accuracy. While introducing a DG-enhanced ViT variant improves in-domain and cross-domain macro performance, the accuracy on the rare class degrades in both settings, despite using the same underlying architecture. Such behavior is unacceptable in high-stakes applications such as autonomous driving, clinical diagnosis, and drone perception, where improvements must be consistent across both frequent and rare classes. (The detailed experimental setup is provided in the appendix 
\ref{table1}.)
...
knowledge integration (CCKI) has a higher likelihood of achieving better
performance than concatenation in MDLT using knowledge-based expert machines.
Building on this theory we propose the \textbf{Agentic Causal Disentanglement
(ACD)} framework, which builds knowledge-guided deep-learning decision trees
to jointly resolve domain and long-tail challenges. ACD automatically
...
Through an agentic, reinforcement-driven pipeline, ACD iteratively refines
...
modalities show that ACD consistently outperforms state-of-the-art DG and
LT baselines and reduces manual annotation burden.
...
This dual-space paradigm is realized by the proposed \textbf{CCKI
algorithm} (Alg.~\ref{alg:ccki}, Fig.~\ref{fig:medxai_framework}), which
integrates visual features from raw data with knowledge features from
structured priors. CCKI builds a class-conditional classifier guided by
\textbf{Expected Information Gain (EIG)} and a \textbf{purity index} for
tree-based partitioning over both spaces. Empirically, CCKI outperforms
...
\end{equation}
while LT weighting magnifies rare-class gradients, emphasizing
\emph{sharp minima}
\begin{equation}
...
%     \caption{\textbf{Overview of the Agentic Causal Disentanglement (ACD)
%     Framework with Class-Conditional Knowledge Integration (CCKI).}
...
%     executable pipelines. The CCKI algorithm iteratively selects classifiers
%     using expected information gain (EIG) to isolate rare classes via
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.99\linewidth]{final-pipeline-1.png}
    \caption{\textbf{Overview of the Agentic Causal Disentanglement (ACD)
    Framework with Class-Conditional Knowledge Integration (CCKI).}
    Three coordinated agents: (1) a Clinical Reasoning Agent retrieves
    evidence and constructs class-conditional causal chains; (2) a Causal
...
\eqref{eqn:CCKI} acts as a path-selector. If $i_t \le m_k$ at step $t$ then
...
\subsection{Instance of CCKI}
\label{sec:ccki_instance}
...
Algorithm~\ref{alg:ccki} provides a practical instantiation of the CCKI framework.
...
CCKI focuses capacity on tail classes while enabling separation between knowledge-driven and data-driven components.
...
impurity measure for deciding whether further cascading is necessary. To justify the design choice, we include ablations that replace EIG with (i) standard information gain (entropy reduction) and (ii) held-out cross-entropy improvement as split criteria, while keeping the CKI/CCKI pipeline fixed (Appendix~\ref{app:eig_baselines}).
...
A central challenge in realizing CCKI is extracting expert knowledge to
...
per attribute. This dependence on fine-grained annotations limits CCKI's
scalability. We address this by developing the \textbf{Agentic Causal
Disentanglement (ACD)} framework, which autonomously constructs these
pipelines, retrieves domain-specific cues, and organizes them into
structured priors for CCKI, reducing manual involvement.

\section{Proposed Method: Agentic Causal Disentanglement}
\label{sec:method}

ACD is a modular agentic pipeline that unifies expert medical knowledge
...
structure, and operationalize clinical knowledge, while the downstream CCKI
...
We now detail the first stages of ACD: acquiring symbolic knowledge,
encoding it into features and rules, and preparing it for use in CCKI
hypotheses $h_K(x)$.

\textbf{Agent~1 (CRA-A): Medical Knowledge Retrieval.}
...
\textbf{Agent~2 (CRA-B): Rule Refinement.}
...
\textbf{Agent~3 (CDA): Architecture Selection.}
...
\textbf{Agent~4 (CGA): Code Generation.}
...
\]
each computing a subset of $\mathcal{K}$. Automated input checks, unit
...
non-improving candidates. The final CCKI cascade therefore uses only
...
We evaluate ACD across four clinically critical applications spanning
neuroimaging, ophthalmology, cardiology, and electrophysiology. Detailed
pipelines, prompts, and implementation are in the supplementary material.
...
classifiers $h_K(x)$ from lesion and vessel-morphology rules. The CDA
...
routines to instantiate them; CCKI organizes $h_D(x)$ and $h_K(x)$ into a
...
(train on three, test on one). Across settings, ACD/CCKI consistently
improves cross-domain accuracy over the ViT-only branch and DG baselines
(Fishr, SPSD-ViT), with $+5$--$6$ point F1 gains for the rare severe DR stages.
...
Performance comparison of our Knowledge-Guided Fusion (ACD) against standard ViT and state-of-the-art DG baselines. 
...
& APTOS & EyePACS, M1, M2 & \textbf{Ours (ACD/CCKI)} & \textbf{Accuracy} & $\mathbf{84.69 \pm 0.3}$ \\
\midrule
\multirow{6}{*}{\textit{SDG}} 
& APTOS & EyePACS, M1, M2 & Ours (ACD Fusion) & Accuracy & $\mathbf{59.9 \pm 0.2}$ \\
& MESSIDOR-1 & APTOS, EyePACS, M2 & Ours (ACD Fusion) & Accuracy & $\mathbf{67.1 \pm 0.4}$ \\
& MESSIDOR-2 & APTOS, EyePACS, M1 & Ours (ACD Fusion) & Accuracy & $\mathbf{65.5 \pm 0.3}$ \\
& EyePACS & APTOS, M1, M2 & Ours (ACD Fusion) & Accuracy & $61.7 \pm 0.4$ \\
...
& E, M1, M2 & APTOS & KL Branch (KL-CCKI) & Accuracy & $53.1 \pm 0.3$ \\
& E, M1, M2 & APTOS & \textbf{Ours (ACD Fusion)} & \textbf{Accuracy} & $\mathbf{60.7 \pm 0.2}$ \\
\midrule
...
& E, M1, M2 & APTOS (Stage 3) & \textbf{Ours (ACD Fusion)} & \textbf{F1 Score} & $\mathbf{50.1 \pm 0.5}$ \\
& E, M1, M2 & APTOS (Stage 4) & DL Branch (ViT) & F1 Score & $51.8 \pm 0.7$ \\
& E, M1, M2 & APTOS (Stage 4) & \textbf{Ours (ACD Fusion)} & \textbf{F1 Score} & $\mathbf{57.3 \pm 0.4}$ \\
\bottomrule
\end{tabular}
\end{table*}
\begin{table*}[h]
...
\caption{Comparison of GRACE with task-specific baselines across modalities. Rare classes denote clinically critical underrepresented conditions.}
\begin{tabular}{
p{2.2cm}
p{1.5cm}
p{2.5cm}
...
\toprule
\textbf{Task} & \textbf{Modality} & \textbf{Dataset} & \textbf{GRACE} & \textbf{Baseline} & \textbf{Base} & \textbf{Gain} & \textbf{Rare Class} \\
\midrule

\rowcolor[gray]{0.95}
SOZ Localization
& MRI
& PCH~\cite{kamboj2024expertTAI}
& \textbf{84.6}
...
& ECG
& MISC~\cite{Banerjee2025StressECG}
& \textbf{92.2}
& Transformer~\cite{Banerjee2025StressECG}
& 82.0
& $\uparrow$ 10.2
& Severe CAD\\
...
\label{tab:grace_results}
\end{table*}
\begin{table*}[h]
\centering
\scriptsize
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.25} % Elegant vertical spacing

\caption{\footnotesize \textbf{Cross-Application Performance and Generalization Gains.} 
Summary of performance improvements using our ACD framework across four medical domains. 
...
\begin{tabular}{lp{2.5cm}p{2.5cm}ccc}
\toprule
\textbf{Application} & \textbf{Source Domain} & \textbf{Target Domain} & \textbf{Baseline} & \textbf{ID Gain} & \textbf{OOD Gain} \\
\midrule

\textit{DR Grading} & APTOS / Messidor & EyePACS / IDRiD & 78.4\% & $+6.90\%$ & $+4.52\%$ \\

\textit{SOZ Localization} & PCH (rs-fMRI) & UNC (rs-fMRI) & 46.1\% & $+38.50\%$ & $+27.65\%$ \\

\textit{Cardiac Function} & EchoNet (Baseline) & EchoNet (Val) & 4.1 (MAE) & $-0.03\%$ & -- \\

\textit{CAD Detection} & MISC ECG (Train) & MISC ECG (Test) & 82.0 (AUC) & $+11.20\%$ & -- \\

\midrule
\rowcolor[gray]{0.95} 
\textbf{Average Improvement} & -- & -- & -- & \textbf{+14.14\%} & \textbf{+16.09\%} \\
\bottomrule
\end{tabular}
\end{table*}
...
\paragraph{Setup.}
For SOZ localization from rs-fMRI, Independent Components (ICs) are first
...
set (K-NumC, K-ThruV, K-SparseA, K-SparseF). The CGA implements the
extraction pipelines and a 2D CNN branch that classifies noise vs.
non-noise ICs. The CDA constructs a CCKI decision tree over $h_K(x)$ and
...
Table~\ref{tab:soz_results} summarizes performance. The fused ACD model
...
\paragraph{Setup.}
We apply ACD to estimating left-ventricular ejection fraction (LVEF) from
...
Because ACD targets generalizable knowledge integration rather than
...
\paragraph{Setup.}
We evaluate ACD on Exercise Stress ECG (ESE) data from the Mayo Integrated
...
detection. ACD integrates cardiologist knowledge through the CRA and CDA
...
To address this we propose Class-Conditional Knowledge Integration (CCKI),
...
rather than naive feature concatenation. Building on CCKI, we introduce
Agentic Causal Disentanglement (ACD), a coordinated multi-agent framework
...
clinical applications and ten datasets, ACD consistently improves
...
\label{proof}
At each depth of the CCKI sequence we either use a data-driven machine or a knowledge-guided machine.
Hence, each depth of CCKI results in a hypothesis with mutually exclusive parameter sets.
...
under the CCKI class of hypotheses. \hfill$\square$
...
domain, the concatenation-based joint can collapse, whereas the CCKI mixture can route to (and be supported by) a higher
...
\paragraph{Why CCKI is not a re-statement of sum/product fusion.}
CCKI does \emph{not} claim novelty in proposing or analyzing generic fixed
fusion operators. Instead, CCKI targets a different problem:
...
every input, CCKI constructs a \emph{routing/cascade policy} that decides
which expert should dominate for specific classes or regions. Concretely,
CCKI uses information-based criteria (Expected Information Gain, EIG) and
...
\begin{center}
    ...
    \captionof{figure}{Comparison of ensemble paradigms: while prior methods (Boosting, Bagging, MoE) lack holistic imbalance awareness, CCKI uniquely integrates representation-aware selection and class-specific routing for robust imbalanced learning.}
    \label{fig:Lorenz}
\end{center}
...
loss, often with sparsity or load-balancing regularizers. CCKI is MoE-like
...
fusion mechanisms as complementary baselines; CCKI emphasizes structured,
...
yielding leaves that store class distributions. While CCKI also employs
...
conventional feature-only decision tree.   
\section{Application Details}
\label{app:applications}
\paragraph{Task-Wise Agent Action Decomposition Across Clinical Domains:} Table~\ref{tab:grace_agent_grounding_summary} summarizes how \texttit{GRACE} integrates retrieval, reasoning, and execution across diverse clinical domains by grounding predictions in domain-specific medical knowledge. Each application encodes structured priors and task-relevant constraints, enabling robust handling of class imbalance, rare cases, and modality-specific variability. This unified agentic design ensures consistent knowledge-guided inference while adapting to heterogeneous data sources and clinical objectives.
\begin{table*}[h]
\centering
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\renewcommand{\arraystretch}{1.25}
\caption{Cross-application summary of \textbf{GRACE}. For each domain we detail the sources retrieved, the clinical knowledge encoded during reasoning, and the computational tools invoked at execution.  Each dataset is partitioned using an 80\% training split and a 20\% testing split.}

\resizebox{\textwidth}{!}{%
\begin{tabular}{p{2.3cm} p{2.8cm} p{4.2cm} p{4.2cm} p{4.2cm}}
\toprule
\textbf{Application} & \textbf{Task} & \textbf{Retrieve Agent} & \textbf{Reason Agent} & \textbf{Execute Agent} \\
\midrule
...
Retrieves Stanford \textbf{EchoNet-Dynamic} dataset paper and associated clinical LVEF reference ranges; fetches ACC/AHA HF guidelines for EF cut-offs ($<$40\%, 40–55\%, $>$55\%) as structured priors &
...
\vspace{-2 pt}
\label{tab:grace_agent_grounding_summary}
\end{table*}
...
ACD does not surpass the hand-engineered EchoNet-Dynamic system, which
benefits from extensive video-specific optimization and years of
domain-tailored refinement. Importantly, ACD provides substantial
...
\captionof{table}{\textbf{SOZ localization on rs-fMRI.} ACD reduces
...
\textit{ACD (Ours)} & \textbf{84.6} & \textbf{89.7} & \textbf{18} \\
\midrule
\rowcolor[gray]{0.95}
...
reported as \emph{internal / external / beat-to-beat (variance)}. ACD
...
\textit{ACD (Ours)}      & 4.8 / 6.6 / 3.1 (5.9) & 0.94 \\
\midrule
\rowcolor[gray]{0.95}
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.15}
\caption{\textbf{CAD detection on the ESE dataset.} ACD integrates expert
priors via lead-selection masks and MET-level encoding, outperforming
both the ViT baseline and manual clinical diagnosis on PPV and AUC.}
...
\textit{ACD (Ours)}           & \textbf{91.2} & \textbf{93.0} & \textbf{92.2} \\
\midrule
\rowcolor[gray]{0.95}
...
A natural concern in method comparison is whether performance gains arise from algorithmic design or from backbone capacity. ERM\_ViT and ERM\_SPSDViT use the same ViT backbone family (DeiT-S / T2T-14) as the deep branch $h_D(x)$ in our model. Our method introduces no additional backbone capacity; instead, it augments the same ViT with knowledge-driven classifiers $h_K(x)$ and a CCKI routing mechanism. Therefore, improvements over ERM baselines are attributable to the integration and routing strategy rather than a stronger feature extractor. We further validate this in Ablation~III (Section~\ref{ablation_study1}), where CCKI is compared against ensemble and mixture-of-experts (MoE) baselines under identical backbone and training configurations, isolating the effect of the routing mechanism.
...
To determine which symbolic knowledge features are suitable for constructing the CCKI knowledge hypothesis $h_{\mathrm{K}}(x)$, we evaluate two clinically motivated feature families on APTOS:
...
This behavior aligns with our theoretical framework: only knowledge features with low class-conditional domain divergence are appropriate for $h_{\mathrm{K}}(x)$ in CCKI, while domain-unstable features increase the effective discrepancy term and weaken rare-class guarantees.
...
This choice provides stable, interpretable, and clinically grounded decision boundaries that integrate reliably with deep hypotheses $h_{\mathrm{D}}(x)$ in the CCKI rule cascade.
...
\end{table}
\vspace{-0.30 in}
\paragraph{Ablation Study II: Effect of Hypothesis Pool Composition in CCKI.}
We investigate how the composition of the CCKI hypothesis pool $\mathcal{H}$ influences in-domain performance on the APTOS dataset for 5-class Diabetic Retinopathy (DR) grading (Stages 0--4). The pool $\mathcal{H}$ consists of two distinct model types: 
...
While $\mathcal{K}$ remains fixed, we systematically vary the number of hypotheses and their prediction granularity (5-class vs.\ binary one-vs-rest). Table~\ref{tab:aptos_ccki_ablation} evaluates six configurations.
...
\bottomrule
\end{tabular}
\caption{Ablation of CCKI hypothesis pool composition on APTOS (5-class DR classification).}
...
Large binary-only mixtures (A3, A4) perform worse due to overlapping or contradictory decision boundaries within the CCKI rule cascade.
Single-family baselines (A5, A6) underperform as they lack complementary perspectives. \textbf{Limitations: }CCKI supports an unbounded number of hypotheses, but effective operation requires avoiding excessive redundancy when using one-vs-rest specialists.
In this work, $|\mathcal{Y}| = 5$ (DR stages), so the binary hypotheses naturally map to five DR subclasses.
Importantly, CCKI is \textit{not restricted to binary splits}: it can accommodate arbitrary sub-multiclass hypotheses (e.g., mild vs.\ severe tiers), which we identify as a promising direction for future work.

\paragraph{Ablation III: Comparison with ensemble and mixture-of-experts baselines.}
\label{app:kittler_moe}
To verify that CCKI's gains are not explained by standard model-combination
...
traditional ensemble achieves 77.90\% accuracy and MoE 81.90\%, while CCKI
reaches \textbf{84.78\%}. This indicates that structured neuro-symbolic
reasoning within the CCKI cascade integrates complementary hypotheses more
...
CCKI outperforms standard ensemble learning and mixture-of-experts.}
...
We evaluate whether EIG is necessary by keeping the CKI/CCKI pipeline unchanged and only swapping the criterion used to
...
\centering
\small
\caption{Eig criterion ablation. We keep cki/ccki fixed and only replace the split/selection criterion.} 
...
target-domain tail robustness and/or more efficient routing (lower KL coverage for the same accuracy).
\newpage
...
\label{app:llm_details}
 ...
 \textbf{LLM backbone.} We use \textit{Gemini 2.5} as the single LLM backend for CRA/CDA/CGA across all experiments to control for model variance and isolate gains from agentic orchestration and CCKI.
...
of $Y$. ACD operationalizes this by producing and selecting
\emph{class-conditional} knowledge features $K(X)$ and combining them
with data-driven predictors via CCKI.
...
We will release the full agentic orchestration code (CRA/CDA/CGA and the CCKI 
cascade), all knowledge-feature extractors (lesion detectors, vessel routines, 
ICA-derived rs-fMRI features, and ECG masking utilities), the CCKI decision logic 
...
ReAct-style, static DAG/LangGraph pipelines, and ACD) is evaluated under an 
...
human-in-the-loop review. We position ACD as a research framework that supports 
...
\scriptsize
    ...
    The framework employs a bipartite training architecture. During inference, the final SOZ classification is determined by integrating the labels from both $\text{M}_{\text{DL}}$ and $\text{M}_{\text{CCKI}}$ via confidence scores, yielding a final, integrated, and explainable diagnostic result.
    }
...
\scriptsize
    ...
    The \textbf{CCKI algorithm} iteratively selects the optimal binary classifier (maximum Entropy Imbalance Gain, EIG) for node splitting.
    }
...
than again agent CRA define rules from expert medical knowledge output by Agent 1 by adapting them to modality constraints and defining thresholds and confidence bounds. 
