...
introduce the \textbf{Retrieve–Reason–Run Agent (R3A)} framework. This modular architecture integrates explicit clinical knowledge sourced
autonomously from sonographers, radiologists, and specialists as a causal
intervention. R3A orchestrates three agents (Clinical knowledge retrieval, Reasoning on Causal
...
\textbf{10 medical-imaging datasets spanning 4 modalities}, R3A yields an
...
knowledge integration (CCKR) has a higher likelihood of achieving better
...
%     \caption{\textbf{Overview of the Agentic Causal Disentanglement (R3A)
%     Framework with Class-Conditional Knowledge Integration (CCKR).}
...
hypotheses $h_K(x)$ as illustrated in figure\label{medxai_framework2}.
...
We evaluate R3A across four clinically critical applications spanning
neuroimaging, ophthalmology, cardiology, and electrophysiology. Detailed
pipelines, prompts, and implementation are in the supplementary material. For each application, we choose datasets that have a rare class, making the dataset long tailed as described in table
...
CCKR uses information-based criteria (Expected Information Gain, EIG) and
...
We evaluate whether EIG is necessary by keeping the CKI/CCKR pipeline unchanged and only swapping the criterion used to
...
\centering
\small
\caption{Eig criterion ablation. We keep cki/CCKR fixed and only replace the split/selection criterion.} 
