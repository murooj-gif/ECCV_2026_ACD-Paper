...
\setcitestyle{round}  %  puts '()' on year- citation 
\usepackage{setspace}
\setstretch{0.94}
...
\title{Resolving the Generalization–Long-Tail Trade-Off via Class-Conditional Knowledge-Guided Agentic Learning}
...
\maketitle
\begin{abstract}
Deep learning models deployed in clinical settings face two major challenges: \textbf{Domain Generalization (DG)} and Long-Tailed Visual Recognition (LT-VR). DG requires learning domain-invariant representations that remain robust across heterogeneous acquisition protocols and patient populations. We formally analyze a fundamental trade-off in which gradient alignment objectives optimized in DG methods conflict with the class-aware optimization required for LT-VR, leading to degraded performance on rare classes. To address this, we propose the \textbf{Agentic Framework Reason–Retrieve–Run (R3A)}, a modular architecture that integrates clinical knowledge and expertise autonomously sourced from sonographers, radiologists, and domain specialists as causal intervention signals, orchestrating three agents: the Clinical Reasoning Agent (CRA), the Causal Disentanglement Agent (CDA), and the Code Generation Agent (CGA), thereby decoupling DG and LT optimization objectives. We evaluate R3A on 10 datasets across four clinical applications and multiple imaging modalities under in-domain, single-domain, and multi-domain generalization settings, where it consistently achieves average gains of 13\% in-domain, 13.82\% out-of-domain, and 13.1\% rare-class F1 over strong DG and long-tail baselines.
\end{abstract}
\section{Introduction}
\label{sec:intro}

As observed by~\cite{Gu22}, the inability of deep-learning models to
...
% \caption{%
%   Comparison of ERM\_ViT,
%   SPSD\_ViT and Ours on Per-class F1 (\%) of Train on APTOS and test on APTOS
...
\caption{%
  Comparison of
  ERM-ViT~\citep{dosovitskiy2021},
  SPSD-ViT~\citep{Su24a},
  and Ours on Per-class F1 (\%).
  Trained on APTOS; tested in-domain
  ($n{=}733$) and cross-domain on
  Messidor ($n{=}1744$).
  \textbf{G4 PDR} = clinically
  critical rare class.
}
\label{tab:perclass_f1}
\resizebox{\linewidth}{!}{%
...
\toprule
\multirow{2}{*}{\textbf{Class}}
  & \multicolumn{2}{c}{\textbf{APTOS (in-domain)}}
  & \multicolumn{3}{c}{\textbf{Messidor (cross-domain)}} \\
\cmidrule(lr){2-3}
\cmidrule(lr){4-6}
  & ERM-ViT
  & SPSD-ViT
...
% \textbf{Contributions.} We formalize and prove that 
...
\begin{itemize}\itemsep2pt \parskip0pt \topsep2pt
  ...
  \item \textbf{Theoretical grounding.}
  We formalize and prove that Knowledge Routing (CCKR)
  achieves a strictly higher likelihood of joint-distribution
  agreement in the target domain than feature-knowledge
  concatenation, and show that it decouples the DG and LT
  optimization objectives at the gradient level.

  \item \textbf{R3A framework.}
  We propose the \textbf{Retrieve--Reason--Run Agent (R3A)},
  a three-agent pipeline (CRA, CDA, CGA) that autonomously
  retrieves clinically validated knowledge from biomedical
  literature, encodes it into computable biomarkers, and
  integrates the result with deep predictors via CCKR --
  requiring no manual knowledge engineering per task.

  \item \textbf{Automated knowledge integration.}
  Through a reinforcement-driven refinement loop, R3A
  generates and validates executable pipelines, retaining
  only knowledge branches that yield measurable gain over
  the deep-learning-only baseline -- cutting expert
  annotation workload by up to 84\% while preserving
  cross-center generalization.

  \item \textbf{Comprehensive empirical validation.}
  Across 10 medical-imaging datasets spanning four modalities,
  R3A consistently outperforms strong DG and LT-VR baselines,
  with average gains of 13\% in-domain, 13.82\%
  out-of-domain, and 13.1\% rare-class F1.
...
as a path in a decision tree as shown in Figure~\ref{fig:medxai_framework2}:
\begin{itemize}
...
(\emph{proof is in Appendix~\ref{proof}}).
...
hypotheses $h_K(x)$ as illustrated in Figure~\ref{fig:medxai_framework2}.
...
maps into a candidate knowledge feature set $\mathcal{K} = \{k_1, k_2, \dots, k_n\},$
...
available data. \textbf{Rule Refinement.}
...
as optic-disc overlap, low contrast, or motion blur. The resulting rule set $\mathcal{R} = \{r_1, r_2, \dots, r_m\}$,
...
classifiers $h_K(x)$ from lesion and vessel-morphology rules described in Section~\ref{app:dr_knowledge}. The CDA
...
knowledge-guided cascade. Table~\ref{tab:dr_all_results} summarizes three regimes: (1) \textbf{Normal
...
\begin{tabular}{ll p{4.3cm} l l c}
\toprule
\textbf{Protocol} & \textbf{Source Domain} & \textbf{Target Domain(s)} & \textbf{Method} & \textbf{Metric} & \textbf{Value (\%)} \\
\midrule
\rowcolor[gray]{0.95} \textit{Normal DG} & APTOS & E, M1, M2 & DL Branch (ViT) & Accuracy & $78.4 \pm 0.4$ \\
& APTOS & E, M1, M2 & \textbf{Ours (R3A/CCKR)} & \textbf{Accuracy} & $\mathbf{84.69 \pm 0.3}$ \\
\midrule
\multirow{6}{*}{\textit{SDG}} 
& APTOS & E, M1, M2 & Ours (R3A Fusion) & Accuracy & $\mathbf{59.9 \pm 0.2}$ \\
& MESSIDOR-1 & APTOS, E, M2 & Ours (R3A Fusion) & Accuracy & $\mathbf{67.1 \pm 0.4}$ \\
& MESSIDOR-2 & APTOS, E, M1 & Ours (R3A Fusion) & Accuracy & $\mathbf{65.5 \pm 0.3}$ \\
...
unseen cases under the same split as prior work, resulting in a 10\%
gain over baselines as shown in Table~\ref{tab:cad_results}.
...
an agentic framework (R3A), a coordinated multi-agent framework
...
burden as shown in Table~\ref{tab:cross_app_summary}. While R3A is validated in high-stakes clinical settings, the framework extends
naturally to any long-tailed, safety-critical domain, such as autonomous driving
and aerospace anomaly detection. Future work will pursue these directions.
...
\caption{Comparison of ensemble paradigms: while prior methods (Boosting, Bagging, MoE) lack holistic imbalance awareness, CCKR uniquely integrates representation-aware selection and class-specific routing for robust imbalanced learning.}
\label{fig:moe}
\end{figure}
...
5-class Diabetic Retinopathy grading under an imbalanced dataset &
...
MISC = Mayo Integrated Stress Center.}
\label{tab:cross_app_summary}
\begin{tabular}{lp{2.8cm}p{3.3cm}ccc}
\toprule
\textbf{Application} & \textbf{Source Domain} & \textbf{Target Domain} & \textbf{Baseline} & \textbf{OOD Gain} & \textbf{F1 OOD Gain} \\
\midrule
\textit{DR Grading}        & APTOS                 & EyePACS, Messidor-1, Messidor-2 & 54.4\% (AUC)   & $+6.30\%$  & $+14.8\%$ \\
\textit{SOZ Localization}  & PCH (rs-fMRI)         & UNC (rs-fMRI)                   & 55.34\% (AUC)  & $+32.16\%$ & $+18.9\%$ \\
\textit{Cardiac Function}  & EchoNet-Dynamic       & Mayo Echocardiogram             & 91.2\% (AUC)     & $+3.00\%$  & $+6.2\%$  \\
\textit{CAD Detection}     & MISC Center 1 (Train) & MISC Center 2 (Test)            & 82.0 (AUC)     & --         & $+12.7\%$ \\
\midrule
\rowcolor[gray]{0.95}
\textbf{Average Improvement} & -- & -- & -- & $+13.82\%$ & $+13.15\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
value (NPV) and area under the curve (AUC).}
...
\paragraph{Ablation IV: EIG Design Choice}
...
\textbf{public} (e.g., EyePACS, APTOS, MESSIDOR, MESSIDOR-2, EchoNet-Dynamic) and
...
library, and performance is evaluated on an unseen target domain that is excluded entirely from training and validation. We adopt DomainBed because it eliminates
